    ...
    public static DownloadManager getDownloadManager(Context appContext) {
        if (!DownloadService.isServiceRunning(appContext)) {
            Intent downloadSvr = new Intent("download.service.action")
                    .setPackage("com.yxst.epic.unifyplatform.download.DownloadService");
        ...
        if (DownloadService.DOWNLOAD_MANAGER == null) {
            ...
            List<XmppMessage> dbMsgs = XmppDbManager.getInstance(appContext)
                    .getMessagesVoiceNotDownloadSuccess(MyApplication.getInstance().getLocalUserName());
            if (dbMsgs != null) {
            	for (XmppMessage m : dbMsgs) {
            		MessageUtils.receiveMediaMsg(appContext, m);
            	}
            }
        }
