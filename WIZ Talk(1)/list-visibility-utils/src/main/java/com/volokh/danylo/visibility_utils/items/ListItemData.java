...
public class ListItemData {
    ...
    public int getVisibilityPercents(List<View> listView,int is) {
        if (listView != null && listView.size() > 0)
            if (listView.get(getIndex()) instanceof ListItem) {
                int visibilityPercents = ((ListItem) (listView.get(getIndex()))).getVisibilityPercents(getView());
                if (SHOW_LOGS)
                    Logger.v(TAG, "getVisibilityPercents, visibilityPercents " + visibilityPercents);
                return visibilityPercents;
            }
